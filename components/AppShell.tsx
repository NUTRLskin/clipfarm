...
// !hydrated guard, a re-emitted session object would keep clobbering the
// mode back to session.user.role, making the Creator toggle appear broken.
useEffect(()=>{
if(hydrated) return;
...
if(stored==="clipper"||stored==="creator"){
resolved=stored;
}else{
const sessionRole=(session?.user as any)?.role as "clipper"|"creator"|undefined;
if(sessionRole==="clipper"||sessionRole==="creator") resolved=sessionRole;
}
}catch{}
